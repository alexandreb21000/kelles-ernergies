<?php /* Template Name: Association */ 
?>


<?php
get_header();
?>

    <div class="w-1200 marg-center d-flex jc-center pad-header">
        <div class="w-20 color-purple d-flex jc-center">
            <p class="rotated color-title playfair fs32 m-none txt-center ">L'ASSOCIATION</p>
        </div>

        <div class="w-80 d-flex ">
            <div class="p-contenu">
                <h2 class="txt-center playfair fs32">LOREM IPSUM</h2>
                <p class="txt-center playfair fs24">Lorem Ipsum</p>
                <hr width="40%">
                <p class="txt-justify noto fs15">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
                Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
                Nulla sit amet nisl nunc. Nullam placerat ipsum id purus aliquet feugiat. Integer tempor felis libero, 
                at tristique turpis aliquet vel. Nulla facilisi. Duis blandit nisl sit amet nunc lobortis sodales.
                Quisque cursus at ex in elementum. Quisque eu lectus et arcu vulputate commodo viverra a ipsum. Maecenas eu elit tellus. 
                </p>
            </div>

            <div>
                <img class="d-block" src="<?php echo THEME_IMG_PATH; ?>/asso2.jpg" alt="association">
            </div>
        </div><!-- #primary -->   
    </div>



    <div class="w-1200 marg-center d-flex flex-rowreverse jc-center bg-footer">
                    <div class="w-20 color-purple d-flex jc-center">
                        <p class="rotated color-title playfair fs32 m-none txt-center ">MISSION</p>
                    </div>
            <div class="w-80 d-flex ai-center m-button">
                <div class=>
                    <!--  <img class="m-55 p-55" src="/wordpress/wp-content/themes/kelles-ernergies/img/mission.jpg" alt=""> -->
                    <img class="" src="<?php echo THEME_IMG_PATH; ?>/mission.JPG" alt="mission">
                </div>
                <div class="p-contenu w-100">
                    <h2 class="txt-center playfair fs32">LOREM IPSUM</h2>
                    <p class="txt-center playfair fs24">Lorem Ipsum</p>
                    <hr width="40%">
                    <p class="txt-justify noto fs15">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
                    Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
                    Nulla sit amet nisl nunc. Nullam placerat ipsum id purus aliquet feugiat. Integer tempor felis libero, 
                    at tristique turpis aliquet vel. Nulla facilisi. Duis blandit nisl sit amet nunc lobortis sodales.
                    Quisque cursus at ex in elementum. Quisque eu lectus et arcu vulputate commodo viverra a ipsum. Maecenas eu elit tellus. 
                    </p>
                </div>
            </div>
    </div>




    <div class="w-1200 marg-center d-flex jc-center">
                    <div class="w-20 color-purple d-flex jc-center">
                        <p class="rotated color-title playfair fs32 m-none txt-center ">ASSEMBLEES GENERALES</p>
                    </div>
            <div class="w-80 d-flex ">
                    <div class="p-contenu">
                        <h2 class="txt-center playfair fs32">LOREM IPSUM</h2>
                        <p class="txt-center playfair fs24">Lorem Ipsum</p>
                        <hr width="40%">
                        <p class="txt-justify noto fs15">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
                        Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
                        Nulla sit amet nisl nunc. Nullam placerat ipsum id purus aliquet feugiat. Integer tempor felis libero, 
                        at tristique turpis aliquet vel. Nulla facilisi. Duis blandit nisl sit amet nunc lobortis sodales.
                        Quisque cursus at ex in elementum. Quisque eu lectus et arcu vulputate commodo viverra a ipsum. Maecenas eu elit tellus. 
                        </p>
                        <p class="txt-center playfair fs24">Lorem Ipsum</p>
                        <hr width="40%">
                        <p class="txt-justify noto fs15">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
                        Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
                        Nulla sit amet nisl nunc. Nullam placerat ipsum id purus aliquet feugiat. Integer tempor felis libero, 
                        at tristique turpis aliquet vel. Nulla facilisi. Duis blandit nisl sit amet nunc lobortis sodales.
                        Quisque cursus at ex in elementum. Quisque eu lectus et arcu vulputate commodo viverra a ipsum. Maecenas eu elit tellus. 
                        </p>
                    </div>
                    
            </div>
    </div>



    <div class="w-1200 marg-center d-flex flex-rowreverse jc-center bg-footer">
                    <div class="w-20 color-purple d-flex jc-center">
                        <p class="rotated color-title playfair fs32 m-none txt-center ">LE BUREAU</p>
                    </div>
            <div class="w-80 d-flex ">
                    <div class="p-contenu">
                        <h2 class="txt-center playfair fs32">LOREM IPSUM</h2>
                        <p class="txt-center playfair fs24">Lorem Ipsum</p>
                        <hr width="40%">
                        <p class="txt-justify noto fs15">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
                        Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
                        Nulla sit amet nisl nunc. 
                        </p>
                        
                        <div class="d-flex jc-center">
                            <!-- <div><img class="m-8 p-8 b-rad50 w-60" src="/wordpress/wp-content/themes/kelles-ernergies/img/mission.jpg" alt=""><p class="txt-justify">Nulla sit amet nisl nun <br>Nulla sit amet nisl nun  </p></div> -->
                            <div class="p-8 d-flex flex-col ai-center"><img class="" src="<?php echo THEME_IMG_PATH; ?>/face.png" alt="face"><p class="txt-justify">Nulla sit amet nisl nun <br>Nulla sit amet nisl nun  </p></div>
                            <div class="p-8 d-flex flex-col ai-center"><img class="" src="<?php echo THEME_IMG_PATH; ?>/face.png" alt="face"><p class="txt-justify">Nulla sit amet nisl nun <br>Nulla sit amet nisl nun</span>  </p></div>
                            <div class="p-8 d-flex flex-col ai-center"><img class="" src="<?php echo THEME_IMG_PATH; ?>/face.png" alt="face"><p class="txt-justify">Nulla sit amet nisl nun <br>Nulla sit amet nisl nun  </p></div>
                        </div>
                    </div>          
            </div>
    </div>

            


    <div class="w-1200 marg-center d-flex jc-center">
                    <div class="w-20 color-purple d-flex jc-center">
                        <p class="rotated color-title playfair fs32 m-none txt-center ">PARTENAIRES</p>
                    </div>
            <div class="w-80 d-flex ">
                    <div class="p-contenu">
                        <h2 class="txt-center playfair fs32">Ils nous font confiance</h2>
                            <p class="txt-justify noto fs15">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
                            Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
                            Nulla sit amet nisl nunc.  
                                <div class="d-flex jc-around">
                                   <!-- <div><img class="m-8 p-8 w-100" src="/wordpress/wp-content/themes/kelles-ernergies/img/loremipsum.jpg" alt=""></div> -->
                                    <img class="" src="<?php echo THEME_IMG_PATH; ?>/loremipsum.jpg" alt="logo_partenaire">
                                    <img class="" src="<?php echo THEME_IMG_PATH; ?>/loremipsum.jpg" alt="logo_partenaire">
                                    <img class="" src="<?php echo THEME_IMG_PATH; ?>/loremipsum.jpg" alt="logo_partenaire">
                                </div>
                            </p>    
                        <h2 class="txt-center playfair fs32">Notre réseau</h2>
                            <div class="d-flex jc-around">
                                <img class="" src="<?php echo THEME_IMG_PATH; ?>/loremipsum.jpg" alt="logo_partenaire">
                                <img class="" src="<?php echo THEME_IMG_PATH; ?>/loremipsum.jpg" alt="logo_partenaire">
                                <img class="" src="<?php echo THEME_IMG_PATH; ?>/loremipsum.jpg" alt="logo_partenaire">
                            </div>              
                    </div>                  
            </div>
            
    </div>

    

<?php
/* get_sidebar(); */
get_footer();

