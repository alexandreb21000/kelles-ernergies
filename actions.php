<?php /* Template Name: Actions */ ?>

<?php get_header();?>

<div class="w-1200 m-auto d-flex d-block-sm jc-center">

    <!-- Titre vertical + background à gauche, disparait sur small medias -->
    <div class="w-20 color-purple d-flex  jc-center noDisplay-sm ">

        <p class=" color-title playfair fs32 m-none txt-center rotated h2-sm">LES ACTIONS</p>
    </div>


    <!-- Titre en responsive -->
    <div class="w-100 color-purple noDisplay-lg ">

        <p class="color-title playfair m-none txt-center lineh150p h150p fs60-sm">LES ACTIONS</p>
    </div>
    
    <div class="w-60 w-80-sm center-sm">
        <div class="p-contenu">
            <h2 class="txt-center playfair fs36 fs40-sm m-none">Lorem Ipsum</h2>
            <p class="txt-center playfair fs24 m-none">Lorem Ipsum</p>
            <hr class="w-40">
            <p class="txt-justify noto fs15 w-80 m-auto">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
            Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
            Nulla sit amet nisl nunc. Nullam placerat ipsum id purus aliquet feugiat. Integer tempor felis libero, 
            at tristique turpis aliquet vel. Nulla facilisi. Duis blandit nisl sit amet nunc lobortis sodales.
            Quisque cursus at ex in elementum. Quisque eu lectus et arcu vulputate commodo viverra a ipsum. Maecenas eu elit tellus. 
            </p>
        </div>
    </div>

    <section class="grid none-sm p-contenu ">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
        <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">      
    </section>
</div>




    <div class="w-1200 m-auto d-flex d-block-sm flex-rowreverse jc-center bgcolor1">
        <div class="w-20 color-purple d-flex jc-center d-block-sm">
            <p class="rotated color-title playfair fs32 m-none txt-center ">AGENDA</p>
        </div>
        <div class="w-80 d-flex d-block-sm">
            <div class="p-contenu">
                
    <!-- C'est ici que sera le calendrier dynamique -->

            </div>                  
        </div>
    </div>



    <div class="w-1200 m-auto d-flex  jc-center d-block-sm">
        <div class="w-20 color-purple d-flex jc-center">
            <p class="rotated color-title playfair fs32 m-none txt-center ">LES ENTREPREN'HEUREUSES</p>
        </div>
        <div class="w-80 d-flex d-block-sm">
            <div class="p-contenu">
            <h2 class="txt-center playfair fs36 fs40-sm m-none">Lorem Ipsum</h2>
            <hr class="w-40">
            <p class="txt-justify noto fs15 w-80 m-auto">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
            Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
            Nulla sit amet nisl nunc. Nullam placerat ipsum id purus aliquet feugiat. Integer tempor felis libero, 
            at tristique turpis aliquet vel. Nulla facilisi. Duis blandit nisl sit amet nunc lobortis sodales.
            Quisque cursus at ex in elementum. Quisque eu lectus et arcu vulputate commodo viverra a ipsum. Maecenas eu elit tellus. 
            </p>
        <section class="grid none-sm p-contenu ">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder">
            <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell pinkBorder"> 
        </section>
            </div>                  
        </div>
    </div>

<div class="w-1200 m-auto d-flex flex-rowreverse jc-center d-block-sm">
        <div class="w-20 color-purple d-flex jc-center">
            <p class="rotated color-title playfair fs32 m-none txt-center ">LES ENTREPREN'HEUREUSES</p>
        </div>
        <div class="w-80 d-flex ">
            <div class="p-contenu">
            <h2 class="txt-center playfair fs36 fs40-sm m-none">Lorem Ipsum</h2>
            <hr class="w-40">
            <p class="txt-justify noto fs15 w-80 m-auto">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed in diam nec purus egestas pretium vel sit amet lectus.
            Vestibulum consequat commodo metus a mollis. Etiam at tellus eget ex imperdiet sagittis quis at ligula. 
            Nulla sit amet nisl nunc. Nullam placerat ipsum id purus aliquet feugiat. Integer tempor felis libero, 
            at tristique turpis aliquet vel. Nulla facilisi. Duis blandit nisl sit amet nunc lobortis sodales.
            Quisque cursus at ex in elementum. Quisque eu lectus et arcu vulputate commodo viverra a ipsum. Maecenas eu elit tellus. 
            </p>
            <div class="d-flex w-60 button-sm-center d-block-sm">
            <img src="<?php echo THEME_IMG_PATH; ?>/kelle_kafe.PNG" alt="kelle_kafe" class="cell ">
                <section class="d-flex none-sm p-contenu">
                    <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell ">
                    <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell">
                    <img src="<?php echo THEME_IMG_PATH; ?>/logo.PNG" alt="img1" class="cell ">
                </section>
            </div>
        </div>                  
    </div>
</div>


<?php get_footer();?>